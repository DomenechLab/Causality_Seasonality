...
fun_PlotClimData = function(loc = names_l[[1]],
  ...
  pl1 <- ggplot(data = clim_dat_log_l[[loc]] %>% 
  ...
  pl5 <- ggplot(data = clim_dat_l[[loc]], aes(x = week_date, y = beta_seas)) + 
    geom_line()
  pl_climate <- pl1 / (pl2 | pl3) / (pl4 | pl5) + plot_annotation(title = loc)
  ggsave(pl_climate, file = glue("_figures/{country}_{loc}_climate.pdf"), height = 5.5)
