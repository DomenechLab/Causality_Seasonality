...
coun_name <- "Colombia"
# Choose 2 locations
if(coun_name == "Spain") covarsnam_2loc <- "LEVS" # Select reference Madrid
...
simulations_trajmatch <- lapply(simulations_l, function(x)
  x %>%
    ...
    select(-week) %>% 
    as.data.frame())
# Bind two locations (one reference)
...
if (locref == "SKRH") {
  ...
  pl <- df_1sim %>%
    ...
    #geom_label(aes(x = -0.0002, y = 20, label = "True value"), 
    #           color = "darkorange", hjust = 0, vjust = 2, label.size = 0) + 
    geom_linerange(aes(xmin = -0.00003, xmax = as.numeric(p_round)), 
                   size = 3, alpha = 0.3, color = "grey20") +
    geom_point(color = "grey20") + 
    scale_y_discrete("") +
    scale_x_continuous("Parameter τ estimate", expand = c(0.0001, 0.0001)) + 
